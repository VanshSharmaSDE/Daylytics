...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // Get all file inline images from inlineImages array
...
router.delete('/:type/:id', auth, async (req, res) => {
  try {
    ...
    if (type === 'task') {
      ...
      // Remove image from inlineImages array
      if (file.inlineImages && file.inlineImages.length > 0) {
        const imageIndex = file.inlineImages.findIndex(img => img.url === imageUrl);
        if (imageIndex !== -1) {
          const removedImage = file.inlineImages[imageIndex];
          file.inlineImages.splice(imageIndex, 1);
          
          if (removedImage.size) {
            user.storageUsed = Math.max(0, user.storageUsed - removedImage.size);
            await user.save();
          }
        }
      }
